"""
Test parallel agent execution via real API call
Tests the coordinator agent's holistic plan creation
"""
...
def get_auth_token():
    ...
    user_data = {
        "email": f"test_parallel_{timestamp}@example.com",
        "username": f"testuser_parallel_{timestamp}",
    ...
    try:
        ...
        if register_response.status_code not in [201, 400]:  # 400 if user exists, that's ok
...
def test_holistic_plan_parallel():
    """Test holistic plan creation with parallel execution"""
    print("=" * 70)
    print("Testing Parallel Agent Execution via API")
    print("=" * 70)
    ...
    if not token:
        print("❌ Authentication failed. Please update credentials in script.")
        print("   Or manually set token variable if you have one.")
        return
    ...
    # Query that should trigger holistic plan
    ...
    print(f"   Endpoint: POST /agents/coordinator/chat/stream")
    ...
    try:
        ...
        if parallel_step in steps_received:
            ...
            print(f"   This indicates all agents are executing simultaneously")
        else:
            print(f"\n⚠️  Parallel execution step not found")
            print(f"   Expected: '{parallel_step}'")
            print(f"   Received steps: {steps_received}")
        
        # Performance analysis
        print(f"\n📊 Performance Analysis:")
        print(f"   Execution time: {execution_time:.2f} seconds")
        if execution_time < 60:
            print(f"   ✅ Time suggests parallel execution (3 agents running simultaneously)")
            print(f"      Sequential execution would typically take 60-90+ seconds")
            print(f"      Each agent makes LLM API calls + tool calls, so ~15-20s per agent")
        else:
            print(f"   ⚠️  Execution time is high")
            print(f"      May indicate slow LLM responses or network latency")
        ...
        if final_response:
            ...
            print(f"   Warnings: {warnings_count} warning(s)")
...
if __name__ == "__main__":
    print("\nNote: This test will auto-register a test user and make a real API call")
    print("      Make sure the backend server is running: uvicorn app.main:app --reload\n")
    test_holistic_plan_parallel()
